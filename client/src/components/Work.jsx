...
export default function Work() {
  const page = usePage("work", { title: "SELECTED\nWORK.", body: "Branding, campaigns, packaging and digital systems." });
  const displayTitle = (page.title || "SELECTED WORK.").includes("\n") ? page.title : (page.title || "SELECTED WORK.").replace(/\s+(?=\S+$)/, "\n");
  const [projects, setProjects] = useState(fallback);
  useEffect(() => { api("/projects").then(setProjects).catch(()=>{}); }, []);

  return (
    <section className="work section" id="work">
      <div className="section-head"><span>01</span><h2>{displayTitle}</h2><p>{page.body}</p></div>
